  ...
  for (let i = 0; i < menuItems.length; i++) {
    ...
    if (!item.image_url) {
      ...
      if (mappings[item.name]) {
        ...
        const cat = categories ? categories.find(c => c.id === item.category_id) : null;
      ...
      console.log('Updating ' + item.name + ' with ' + cover);
      await supabase.from('menu_items').update({ image_url: cover }).eq('id', item.id);
    }
  }
